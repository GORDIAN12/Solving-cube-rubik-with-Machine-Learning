...
class Cube:
    ...
    def rotate(self, axis, theta):
        #crear la rotacion de matriz basada en ejes especificos
        if axis==0:
        ...
        #aplicar la rotacion a la matriz y actualizar posicion
    ...
    def get_rotation_axis_angle(self):
        ...
        if angle==0:
            ...
            rz=self.orientation[0,1] - self.orientation[1,0]
    ...
    def handle_rotation(self, rotation_queue, animation_step=None):
        ...
        if self.is_rotating:
            if(self.rotation_angle != self.target_rotation):
                ...
                #incrementar la rotacion del angulo en la direccion correcta
                self.rotation_angle+= delta_angle if self.target_rotation>0\
                                                    else -delta_angle
            else:
                delta_angle=0

                #para la rotacion cuando el objetivo de rotacion se alcanzo
                self.is_rotating=False
